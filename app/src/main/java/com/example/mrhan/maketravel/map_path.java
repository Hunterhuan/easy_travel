...
import com.example.mrhan.maketravel.Adapter.RouteLineAdapter;
...
public class map_path extends Activity implements BaiduMap.OnMapClickListener,
    ...
    static int theday = 1;
    List<Integer> dayrecord = new ArrayList<Integer>();
    ...
    protected List<List<String>> getDays (ArrayList<String> dayroutes) {
        List<List<String>> DaysSpots= new ArrayList<List<String>>();
        List<String> OneDay = new ArrayList<String>();
        for (String item : dayroutes) {

            if (item.startsWith("Day") && !OneDay.isEmpty()) {
                List<String> tempday = new ArrayList<String>();
                tempday = (List<String>) ((ArrayList<String>) OneDay).clone();
                DaysSpots.add(tempday);
        ...
        DaysSpots.add(OneDay);
        System.out.println(DaysSpots);
        return DaysSpots;
    }
     ...
     * @param v
     */
    public void searchButtonProcess(View v) throws InterruptedException {
        // 重置浏览节点的路线数据
        route = null;
        mBtnPre.setVisibility(View.INVISIBLE);
        mBtnNext.setVisibility(View.INVISIBLE);
        mBaidumap.clear();
        List<List<String>> mydays = getDays(Dayroutes);

        int dayNum = mydays.size();
        // 处理搜索按钮响应
    ...
    public void nodeClick(View v) {
        ...
        // show popup
        popupText = new TextView(map_path.this);
    ...
    public void onGetWalkingRouteResult(WalkingRouteResult result) {
        if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
            Toast.makeText(map_path.this, "抱歉，未找到结果", Toast.LENGTH_SHORT).show();
        }
        if (result.error == SearchResult.ERRORNO.AMBIGUOUS_ROURE_ADDR) {
            // 起终点或途经点地址有岐义，通过以下接口获取建议查询信息
            // result.getSuggestAddrInfo()
            AlertDialog.Builder builder = new AlertDialog.Builder(map_path.this);
            builder.setTitle("提示");
        ...
        if (result.error == SearchResult.ERRORNO.NO_ERROR) {
            ...
            if (result.getRouteLines().size() > 1) {
                ...
                if (!hasShownDialogue) {
                    MyTransitDlg myTransitDlg = new MyTransitDlg(map_path.this,
...
/*            if (result.getRouteLines().size() > 1) {
            ...
            if (result.getRouteLines().size() >= 1) {
                ...
                System.out.println(theday);
                System.out.println(day);
                DrivingRouteOverlay overlay = new MyDrivingRouteOverlay(mBaidumap, theday);
                if (dayoverlay == dayrecord.get(theday - 1) - 1){
                    theday = (theday + 1) % day;
                    dayoverlay = 0;
                    if (theday == 0)
    ...
    private class MyDrivingRouteOverlay extends DrivingRouteOverlay {
        ...
        int theday;
        public MyDrivingRouteOverlay(BaiduMap baiduMap, int day) {
            super(baiduMap, day);
            //System.out.println(theday);
            theday = day;
        }
